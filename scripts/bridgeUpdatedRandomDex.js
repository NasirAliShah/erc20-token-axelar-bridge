...
const ethRandomDEXTokenABI = require("../utils/EthUpdatedRandomDexABI.json");
...
const ethRandomDEXTokenAddress = process.env.ETH_UPDATED_RANDOMDEX_CONTRACT_ADDRESS; // Ethereum updated contract address
...
async function gasEstimatorForEth() {
  try {
    ...
    const gmpParams = {
      ...
      sourceContractAddress: process.env.ETH_UPDATED_RANDOMDEX_CONTRACT_ADDRESS,
      tokenSymbol: "ETH",
...
async function deployRemoteTokenManager() {
  try {
    ...
    const deployTx = await interchainTokenServiceContract.deployTokenManager(
      process.env.UPDATED_TOKEN_SALT, // Salt from Base deployment
      "ethereum-sepolia",
      MINT_BURN,
      params,
      gasAmount,
    ...
    const tokenId = await interchainTokenServiceContract.interchainTokenId(
      signer.address,
      process.env.UPDATED_TOKEN_SALT
    );
...
async function transferMintAccessToTokenManagerOnEth() {
  try {
    ...
    const grantMinterTx = await tokenContract.grantRole(
      minterRole,
      process.env.UPDATED_TOKEN_MANAGER_ETH_ADDRESS // Token Manager Address for Ethereum
    );
